...
function seeItInActionSectionHtml() {
  const cards = SERVICES_SEE_IT_IN_ACTION_LINKS.map((link) =>
    caseStudyCardHtml(link.href, link.label),
  ).join("\n");

  return `<section class="case-study-links scroll-mt-28" aria-labelledby="services-see-it-in-action-heading">
  <div class="flex flex-col gap-5 md:flex-row md:items-end md:justify-between md:gap-8">
    <div class="max-w-2xl">
      ...
      <h2 id="services-see-it-in-action-heading" class="mt-2 font-heading text-[28px] font-bold leading-[1.1] text-[#0f2e58] md:text-[34px]">See It In Action</h2>
      <p class="mt-3 text-[16px] leading-[1.65] text-[#4a5d75] md:text-[17px]">${escapeHtml(SERVICES_SEE_IT_IN_ACTION_SUBHEADING)}</p>
    </div>
...
function furtherReadingSectionHtml() {
  const cards = SERVICES_FURTHER_READING_LINKS.map((link) =>
    featuredBlogCardHtml(link.href, link.label),
  ).join("\n");

  return `<section class="further-reading scroll-mt-28 border-t border-[#dce3ec]/80 pt-12 md:pt-14 lg:pt-16" aria-labelledby="services-further-reading-heading">
  <div class="flex flex-col gap-5 md:flex-row md:items-end md:justify-between md:gap-8">
    <div class="max-w-2xl">
      <p class="text-[12px] font-bold uppercase tracking-[0.14em] text-[#2e5b88]">Insights</p>
      <h2 id="services-further-reading-heading" class="mt-2 font-heading text-[28px] font-bold leading-[1.1] text-[#0f2e58] md:text-[34px]">Related Readings</h2>
      <p class="mt-3 text-[16px] leading-[1.65] text-[#4a5d75] md:text-[17px]">${escapeHtml(SERVICES_FURTHER_READING_SUBHEADING)}</p>
    </div>
...
export function servicesRelatedContentToHtml() {
  return `<div class="services-related-content relative overflow-hidden border-t border-[#dce3ec] bg-gradient-to-b from-[#f4f8fc] via-[#f8fafc] to-[#eef2f6]" aria-label="Case studies and articles">
