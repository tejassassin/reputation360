...
/** 2–3 characters for a circular avatar, ignoring common honorifics */
...
function SpotlightCard({ t, quoteId, reduce }) {
  return (
    <motion.div
      {...fadeUp(reduce)}
      className="relative"
    >
      <div
        className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-navy/90 via-navy to-[#152E52] p-[1px] shadow-[0_32px_64px_-32px_rgba(15,35,60,0.55)]"
      >
...
function MasonryCard({ t, reduce }) {
  return (
    <li className="break-inside-avoid mb-5 [page-break-inside:avoid]">
      <motion.article
        {...fadeUp(reduce)}
...
export function Testimonials() {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="absolute -left-1/3 top-0 h-[32rem] w-[70%] rounded-full bg-gradient-to-r from-navy/8 to-transparent blur-3xl"
        />
        <div
          className="absolute -right-1/4 top-1/2 h-80 w-[50%] rounded-full bg-gradient-to-l from-[#2E5B88]/10 to-transparent blur-3xl"
        />
        <div
      ...
      <div className="relative z-[2]">
        ...
        <div className="mx-auto max-w-6xl text-left">
          <div className="mb-12 sm:mb-16 md:mb-20" aria-describedby={quoteId}>
            <SpotlightCard
              t={spotlight}
              quoteId={quoteId}
              reduce={reduce}
          ...
          <div className="md:hidden">
            <Scroller>
              {stream.map((t) => (
                <StreamCard key={t.id} t={t} reduce={reduce} />
              ))}
            </Scroller>
          </div>
          ...
          >
            {stream.map((t) => (
              <MasonryCard key={t.id} t={t} reduce={reduce} />
            ))}
          </ul>
        </div>
      </div>
