  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="mt-1 text-left text-[0.7rem] text-steel md:hidden sm:text-[0.7rem]">
          Swipe the row to see every period. Connectors show progress.
        </p>

        <aside
          className="mt-5 rounded-2xl border border-slate/15 bg-gradient-to-br from-offwhite to-white p-4 shadow-sm md:mt-6 md:p-6"
          aria-live="polite"
        >
          <div className="flex flex-col gap-3 border-b border-slate/15 pb-4 sm:flex-row sm:items-center sm:gap-4">
            <span className="grid h-12 w-12 shrink-0 place-items-center self-start rounded-xl bg-slate/10 text-navy ring-2 ring-navy sm:h-14 sm:w-14">
            ...
            <span className="inline-flex w-fit items-center justify-center rounded-full bg-white px-3 py-1.5 font-heading text-xs font-bold text-navy ring-1 ring-slate/15 tabular-nums sm:ml-auto">
              {active + 1} / {n}
            </span>
          </div>
          {cur?.text ? (
            <p className="mt-3 text-[0.95rem] leading-relaxed text-steel [text-wrap:pretty] sm:mt-4 sm:text-base sm:leading-relaxed">
              {cur.text}
            </p>
          ) : null}
        </aside>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-start gap-2.5 md:mt-5">
          <button
            type="button"
            className="inline-flex items-center gap-1.5 rounded-full border border-slate/15 bg-white px-3.5 py-2.5 text-sm font-semibold text-navy shadow-sm transition-colors hover:border-navy/30 hover:bg-offwhite disabled:pointer-events-none disabled:opacity-40"
            disabled={active === 0}
