...
export function WhoWeServeAudienceCard({
  ...
  return (
    <li className={["list-none h-full min-h-0", className].filter(Boolean).join(" ")}>
      <a href={href} {...internalAnchorProps(href)} className={cardClassName}>
        ...
        <div className="relative flex items-center justify-between gap-3">
          <div className="inline-flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl border border-green/30 bg-navy/80 text-green shadow-sm shadow-navy/40 transition group-hover:border-green/55 sm:h-14 sm:w-14 sm:rounded-2xl">
        ...
        <div className="relative mt-4 border-t border-white/15 pt-4">
          <h3 className="font-heading mb-2.5 text-lg font-bold leading-snug text-white [text-shadow:0_1px_2px_rgba(0,0,0,0.2)] sm:text-xl">
            {title}
          </h3>
          <p className="mb-0 text-sm font-medium leading-relaxed text-slate-100/95 [text-shadow:0_1px_2px_rgba(0,0,0,0.2)] sm:text-[15px]">
            {description}
          </p>
        </div>
      </a>
